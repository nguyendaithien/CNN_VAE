...
static void convolution( int padding, int width, int hight ,INPUT_DATA_TYPE bias[CONV_NUM_CHANNEL], data_t *image, const KERNEL_TYPE *kernel,OUTPUT_DATA_TYPE *output) {
 ...
 VITIS_LOOP_39_1: for ( int num_channel = 0 ; num_channel < CONV_NUM_CHANNEL ; num_channel ++ ){
      VITIS_LOOP_40_2: for( int num_ker= 0 ; num_ker < CONV_NUM_KERNEL ; num_ker ++){
Push_pixel: for( int i = 0 ; i < hight ; i++ ) {
    VITIS_LOOP_42_3: for( int j = 0 ; j < width; j++ ) {
...
Shift_win_right:for( int b = 0 ; b < width - 2 ; b++) {
        VITIS_LOOP_65_4: for( int n = 0 ; n < 3; n++) {
...
INPUT_DATA_TYPE sum = 0;
          VITIS_LOOP_81_5: for (int m = 0; m < KERNEL_XSIZE; m++) {
             VITIS_LOOP_82_6: for (int n = 0; n < KERNEL_YSIZE; n++) {
...
static void max_pooling(int width, int height, DATA_IN_TYPE *input, DATA_OUT_TYPE *output) {
    ...
    VITIS_LOOP_115_1: for (int num_channel = 0; num_channel < NUM_CHANNEL; num_channel++) {
        VITIS_LOOP_116_2: for (int i = 0; i < height; i += STRIDE_SIZE) {
            VITIS_LOOP_117_3: for (int j = 0; j < width; j += STRIDE_SIZE) {
                VITIS_LOOP_118_4: for (int k = 0; k < POOLING_SIZE; k++) {
                ...
                if (i + POOLING_SIZE - 1 < height && j + POOLING_SIZE - 1 < width) {
                    ...
                    VITIS_LOOP_125_5: for (int a = 0; a < POOLING_SIZE; a++) {
                        VITIS_LOOP_126_6: for (int b = 0; b < POOLING_SIZE; b++) {
...
static void upsampling(int width, int height, DATA_IN_TYPE* input, DATA_OUT_TYPE* output) {
    ...
    VITIS_LOOP_148_1: for (int num_channel = 0; num_channel < NUM_CHANNEL; num_channel++) {
        VITIS_LOOP_149_2: for (int i = 0; i < UPSAMPLING_FACTOR_X * height; i++) {
            VITIS_LOOP_150_3: for (int j = 0; j < UPSAMPLING_FACTOR_Y * width; j++) {
                if (i % 2 == 0) {
                    if (j % 2 == 0) {
...
convolution<data_t,data_t,data_t,data_t,14,14,1,8,14,14,3,3,1,1>(padding, width, hight,bias_conv2,output_pooling1, kernel_conv2, output_conv2);
